...
csvwrite([data_dir,'GeneAssociation_Main\PLS_stats.csv'],myStats);

%% Draw variance explanation
load([data_dir,'GeneAssociation_Main\PLS_stats.csv']);
...
save([data_dir,'GeneAssociation_Main\PLS1_ROIscore.mat'],'PLS1_ROIscores_280');
...
fid1 = fopen([data_dir,'GeneAssociation_Main\PLS1_geneWeights.csv'],'w');
...
save([data_dir,'GeneAssociation_Main\PLS1_weight.txt'],'PLS1_weight','-ascii');

PLS2_weight = zeros(length(ParcelLabel),1);
for i = 1:280
    PLS2_weight(ParcelLabel==region_ind(i)) = PLS2_ROIscores_280(i);
end
save([data_dir,'GeneAssociation_Main\PLS2_weight.txt'],'PLS2_weight','-ascii');
...
p(2) = length(find(corr_surr(2,:)>corr_real(2)))/10000;
save([data_dir,'GeneAssociation_Main\corr_surr.mat'],'corr_surr','corr_real','p');
...
linecolor = [0 0 0];
ylable1 = 'PLS1 scores';
xlable1 = {'{\itZ}-statistic of';'case-control gradient difference'};
[xData, yData] = prepareCurveData(gradients(region_ind), PLS1_ROIscores_280);
ft = fittype( 'poly1' );
opts = fitoptions( ft );
opts.Lower = [-Inf -Inf];
opts.Upper = [Inf Inf];
...
set(gca,'YLim',[-0.16,0.2]);
set(gca,'YTick',[-0.16:0.09:0.2]);
t1 = text(-4,0.17,{'{\itr} = 0.574';'{\itP} < 0.0001'},'FontName','Arial','FontSize',6);
set(gcf, 'PaperPositionMode', 'manual');
set(gcf, 'PaperUnits', 'centimeters');
set(gcf,'Paperposition',[0 0 3.95 3.8]);
grid off
box off
print(gcf,[figure_dir,'corr_PLS1_g1_main.tif'],'-dtiff','-r1000')
...
ylable1 = 'PLS2 scores';
xlable1 = {'{\itZ}-statistic of';'case-control gradient difference'};
[xData, yData] = prepareCurveData(gradients(region_ind), PLS2_ROIscores_280);
ft = fittype( 'poly1' );
opts = fitoptions( ft );
opts.Lower = [-Inf -Inf];
opts.Upper = [Inf Inf];
...
set(gca,'YLim',[-0.16,0.2]);
set(gca,'YTick',[-0.16:0.09:0.2]);
t1 = text(-4,0.17,{'{\itr} = 0.456';'{\itP} < 0.0001'},'FontName','Arial','FontSize',6);
set(gcf, 'PaperPositionMode', 'manual');
set(gcf, 'PaperUnits', 'centimeters');
set(gcf,'Paperposition',[0 0 3.95 3.8]);
grid off
box off
print(gcf,[figure_dir,'corr_PLS2_g1_main.tif'],'-dtiff','-r1000')