...
for j = 1:10
    num = ['0',num2str(j)];
    % select subject
    sub_id = find(sub_info(:,2)~=j);
    des = [sub_info(sub_id,1),sub_info(sub_id,3),sub_info(sub_id,4)];
    id_hc = find(sub_info(:,1)==1&sub_info(:,2)~=j);
    id_mdd = find(sub_info(:,1)==2&sub_info(:,2)~=j);
    n_mdd = length(id_mdd);
    n_hc = length(id_hc);
    disp([n_mdd,n_hc]);
    % explained ratio
    ...
    disp(stat_std)
    mkdir([data_dir,'Validation_LeaveOneSiteOut\site',num(end-1:end)]);
    save([data_dir,'Validation_LeaveOneSiteOut\site',num(end-1:end),'\Topography_stat.mat'],'stat_exp','stat_range','stat_exp');
end
...
for k = 1:10
    ...
    cd([data_dir,'Validation_LeaveOneSiteOut\site',num(end-1:end)]);
