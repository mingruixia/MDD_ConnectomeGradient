...
% select subject
id_med = find(sub_info(:,1)==2&sub_info(:,10)==1);
id_nomed = find(sub_info(:,1)==2&sub_info(:,10)==0);

sub_id = [id_nomed;id_med];

n_med = length(id_med);
n_nomed = length(id_nomed);

des = [[ones(n_nomed,1);zeros(n_med,1)],sub_info(sub_id,3),sub_info(sub_id,4)];

% explained ratio
stat_exp = zeros(3,7);
for i = 1:3
    stat_exp(i,1) = mean(exprate_corrected(id_med,i));
    stat_exp(i,2) = std(exprate_corrected(id_med,i));
    stat_exp(i,3) = mean(exprate_corrected(id_nomed,i));
    stat_exp(i,4) = std(exprate_corrected(id_nomed,i));    
    stat_result = regstats(exprate_corrected(sub_id,i),des,'linear',{'tstat','r'});
    stat_exp(i,5) = stat_result.tstat.t(2);
    stat_exp(i,6) = stat_result.tstat.t(2) * sqrt(1/n_med + 1/n_nomed);
...
for i = 1:3
    stat_range(i,1) = mean(emb_range(id_med,i));
    stat_range(i,2) = std(emb_range(id_med,i));
    stat_range(i,3) = mean(emb_range(id_nomed,i));
    stat_range(i,4) = std(emb_range(id_nomed,i));    
    stat_result = regstats(emb_range(sub_id,i),des,'linear',{'tstat','r'});
    stat_range(i,5) = stat_result.tstat.t(2);
    stat_range(i,6) = stat_result.tstat.t(2) * sqrt(1/n_med + 1/n_nomed);
...
stat_std = zeros(3,7);
for i = 1:3
    stat_std(i,1) = mean(emb_std(id_med,i));
    stat_std(i,2) = std(emb_std(id_med,i));
    stat_std(i,3) = mean(emb_std(id_nomed,i));
    stat_std(i,4) = std(emb_std(id_nomed,i));    
    stat_result = regstats(emb_std(sub_id,i),des,'linear',{'tstat','r'});
    stat_std(i,5) = stat_result.tstat.t(2);
    stat_std(i,6) = stat_result.tstat.t(2) * sqrt(1/n_med + 1/n_nomed);
    stat_std(i,7) = stat_result.tstat.pval(2);
end
disp(stat_std)

save([data_dir,'BetweenGroupDiff_Medicated\Topography_stat.mat'],'stat_exp','stat_range','stat_exp');
...
cd([data_dir,'BetweenGroupDiff_Medicated']);
