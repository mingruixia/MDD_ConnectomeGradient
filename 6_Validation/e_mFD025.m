...
% select subject
sub_id = find(sub_info(:,12)<0.25);
des = [sub_info(sub_id,1),sub_info(sub_id,3),sub_info(sub_id,4)];
id_hc = find(sub_info(:,1)==1&sub_info(:,12)<0.25);
id_mdd = find(sub_info(:,1)==2&sub_info(:,12)<0.25);
n_mdd = length(id_mdd);
n_hc = length(id_hc);
disp([n_mdd,n_hc]);
% explained ratio
...
disp(stat_std)
mkdir([data_dir,'Validation_mFD025']);
save([data_dir,'Validation_mFD025\Topography_stat.mat'],'stat_exp','stat_range','stat_exp');
