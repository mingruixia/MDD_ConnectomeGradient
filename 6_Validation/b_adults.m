...
% select subject
sub_id = find(sub_info(:,3)>=18);
des = [sub_info(sub_id,1),sub_info(sub_id,3),sub_info(sub_id,4)];
id_hc = find(sub_info(:,1)==1&sub_info(:,3)>=18);
id_mdd = find(sub_info(:,1)==2&sub_info(:,3)>=18);
n_mdd = length(id_mdd);
n_hc = length(id_hc);
disp([n_mdd,n_hc]);
% explained ratio
...
disp(stat_std)
mkdir([data_dir,'Validation_adult']);
save([data_dir,'Validation_adult\Topography_stat.mat'],'stat_exp','stat_range','stat_exp');
