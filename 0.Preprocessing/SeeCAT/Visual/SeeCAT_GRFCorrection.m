function varargout = SeeCAT_GRFCorrection(varargin)
% SEECAT_GRFCORRECTION MATLAB code for SeeCAT_GRFCorrection.fig
%      SEECAT_GRFCORRECTION, by itself, creates a new SEECAT_GRFCORRECTION or raises the existing
%      singleton*.
%
%      H = SEECAT_GRFCORRECTION returns the handle to a new SEECAT_GRFCORRECTION or the handle to
%      the existing singleton*.
%
%      SEECAT_GRFCORRECTION('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in SEECAT_GRFCORRECTION.M with the given input arguments.
%
%      SEECAT_GRFCORRECTION('Property','Value',...) creates a new SEECAT_GRFCORRECTION or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before SeeCAT_GRFCorrection_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to SeeCAT_GRFCorrection_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help SeeCAT_GRFCorrection

% Last Modified by GUIDE v2.5 22-Jun-2016 21:47:02

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @SeeCAT_GRFCorrection_OpeningFcn, ...
                   'gui_OutputFcn',  @SeeCAT_GRFCorrection_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before SeeCAT_GRFCorrection is made visible.
function SeeCAT_GRFCorrection_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to SeeCAT_GRFCorrection (see VARARGIN)
...
% Choose default command line output for SeeCAT_GRFCorrection
handles.OverlayHeader=OverlayHeader;
% Update handles structure
guidata(hObject, handles);

% UIWAIT makes SeeCAT_GRFCorrection wait for user response (see UIRESUME)
uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = SeeCAT_GRFCorrection_OutputFcn(hObject, eventdata, handles) 
