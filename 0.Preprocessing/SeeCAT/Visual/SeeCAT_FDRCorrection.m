function varargout = SeeCAT_FDRCorrection(varargin)
% SEECAT_FDRCORRECTION MATLAB code for SeeCAT_FDRCorrection.fig
%      SEECAT_FDRCORRECTION, by itself, creates a new SEECAT_FDRCORRECTION or raises the existing
%      singleton*.
%
%      H = SEECAT_FDRCORRECTION returns the handle to a new SEECAT_FDRCORRECTION or the handle to
%      the existing singleton*.
%
%      SEECAT_FDRCORRECTION('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in SEECAT_FDRCORRECTION.M with the given input arguments.
%
%      SEECAT_FDRCORRECTION('Property','Value',...) creates a new SEECAT_FDRCORRECTION or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before SeeCAT_FDRCorrection_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to SeeCAT_FDRCorrection_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help SeeCAT_FDRCorrection

% Last Modified by GUIDE v2.5 22-Jun-2016 21:47:32

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @SeeCAT_FDRCorrection_OpeningFcn, ...
                   'gui_OutputFcn',  @SeeCAT_FDRCorrection_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before SeeCAT_FDRCorrection is made visible.
function SeeCAT_FDRCorrection_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to SeeCAT_FDRCorrection (see VARARGIN)
...
% Choose default command line output for SeeCAT_FDRCorrection
handles.OverlayHeader=OverlayHeader;
% Update handles structure
guidata(hObject, handles);

% UIWAIT makes SeeCAT_FDRCorrection wait for user response (see UIRESUME)
uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = SeeCAT_FDRCorrection_OutputFcn(hObject, eventdata, handles) 
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
DPABIPath=fileparts(which('SeeCAT.m'));
...
if ~isempty(P)
    ...
    Header.PMin=Header.PMax;
    Header.NMin=Header.NMax;
